pro plotsizemass, lgnlist, explist, $
...
;  stop
  ...
  ysize = 7
  ar = ysize / xsize
  device, filename = 'sizeMassPlus.eps', $
...
;        ytitle = 'log !18r!X!De, intrinsic!N [kpc]', $
        pos = [0.15,0.20,0.5,0.9] + [0,0,0.025,0]
  cgtext, 0.06, mean(!Y.WINDOW), $
          'log !18r!X!De, intrinsic!N [kpc]', $
          charsize = 1.5, charthick = 4, align = 0.5, orien = 90, /norm
...
;              oob_low = 240, oob_hi = 10
  legend, pos = [10.75,0], /data, box = 0, $;/horiz, $
...
;     oplot, [imasses[*,1]], [bts[*,1]], psym = 8, symsize = 1, col = cgcolor(string(cols[ii]))
  ...
  axis, yaxis = 1, $
        ythick = 3, ytickname = replicate(' ', 60), $
        yminor = 2, ytickint = 0.1, yran = !Y.CRANGE, /ysty
  ...
  cgtext, mean([0.1,0.9]), !Y.WINDOW[0] - 0.1, 'log !18M!X!D*!N [M!D'+sunsymbol()+'!N]', $
          charsize = 1.5, charthick = 5, align = 0.5, /norm
  
  device, /close
  set_plot, 'X'
  spawn, 'gv sizeMassPlus.eps &'
...
;plotsizemass, 'study5_lgn_Bestfits.list', 'study5_exp_Bestfits.list', /circularize
