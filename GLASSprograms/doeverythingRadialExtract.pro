...
pro doselect, cLevel = cLevel, $
  ...
  if NOT keyword_set(cLevel) then docontam = 0
  if NOT keyword_set(minMag) then minMag = 14.
  if NOT keyword_set(maxMag) then minMag = 21.8
  if NOT keyword_set(zMin)   then zMin   = 1.0
  if NOT keyword_set(zMax)   then zMax   = 1.8
  
  spawn, 'ls ~/Projects/GLASS/MasterCats_V1/*allAvailableData.fits > catalogs.list'
...
pro doFinal
  ...
  ;; This was used 2017 05 26 to get the "final" analysis sample
  
  doselect, zmin = 1.0, zmax = 1.8, minmag = 14., maxmag = 21.8, /delete_prev
